...
import {INFINITE_SCROLLING_PAGINATION_RESULTS, QueryKeys} from "@/configs/constants";
...
const PostFeed: FC<Props> =  ({initialPosts, subredditName}) => {
    ...
    const {data: infiniteData, fetchNextPage, isFetchingNextPage} = useInfiniteQuery([QueryKeys.INFINITY_QUERY], async ({pageParam = 1}) => {
        ...
        return data
    }, {getNextPageParam: (_, pages) => pages.length + 1, initialData: {pages: [initialPosts], pageParams: [1]}})
    ...
    return (
        <ul className='flex flex-col col-span-2 space-y-6'>
            {posts.map((post, index) => {
                ...
                return <li key={post.id} {...(index === posts.length - 1 && {ref: ref})}>
                        <Post />
                    </li>
